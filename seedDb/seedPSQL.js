...
async function seedLanguages() {
  return new Promise(async (resolve, reject) => {
    ...
    for (const language of allLanguages) {
      const languageResult = await database.relations.languages.addLanguages(
        language
      );
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const collection of allCollections) {
      const collectionResult =
        await database.relations.collections.addCollection(collection);
...
async function seedSubjects() {
  return new Promise(async (resolve, reject) => {
    ...
    for (const subject of allSubjects) {
      const subjectResult = await database.relations.subjects.addSubjects(
        subject
      );
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const contributor of allContributors) {
      const contributorResult =
        await database.relations.contributors.addContributor(contributor);
...
async function seedPublishers() {
  return new Promise(async (resolve, reject) => {
    ...
    for (const publisher of allPublishers) {
      const publisherResult = await database.relations.publishers.addPublisher(
        publisher
      );
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const book of booksFromCSV) {
      if (bookCounter >= 42000) {
        ...
        const booksResult = await database.relations.books.addBook(
          booksRelation
        );
        ...
        for (const bookLanguage of book.languages) {
          ...
          const booksLanguageResult =
            await database.relations.books_languages.addBooksLanguages(
              bookLanguageRelation
            );
        ...
        for (const bookSubject of book.subjects) {
          const subjectIDFromDB =
            await database.relations.subjects.getSubjectIdBySubject(
              bookSubject
            );
          ...
          const booksSubjectResult =
            await database.relations.books_subjects.addBooksSubjects(
              bookSubjectRelation
            );
        ...
        for (const bookLCSH of book.subject_lcsh) {
          ...
          const booksLCSHResult =
            await database.relations.books_lcsh.addBookLcsh(bookLCSHRelation);
          // console.log(booksSubjectResult.rows[0]);
        }
        ...
        for (const bookPublishedBy of book.digital_publisher) {
          ...
          const bookPublisherResult =
            await database.relations.books_published_by.addBookPublishedBy(
              bookPublisherRelation
            );
          // console.log(booksLanguageResult.rows[0]);
        }
        ...
        for (const bookCollection of book.collections) {
          ...
          const booksCollectionResult =
            await database.relations.books_in_collections.addBookInCollections(
              bookCollectionRelation
            );
          // console.log(booksSubjectResult.rows[0]);
        }
        ...
        for (const altTitle of book.alternative_titles) {
          ...
          const altTitleResult =
            await database.relations.alternative_titles.addAltTitles(
              altTitleRelation
            );
          // console.log(booksSubjectResult.rows[0]);
        }
        ...
        for (const bookContributor of book.contributors) {
          ...
          const bookContributorResult =
            await database.relations.books_contributors.addBookContributor(
              bookContributorRelation
            );
          // console.log(booksSubjectResult.rows[0]);
        }
