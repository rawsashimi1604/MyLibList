...
async function addReadingList(readingList) {
  ...
  try {
    ...
    const updateReadingListID = await db
      .collection("reading_lists")
      .updateOne(
...
async function checkBookInReadingListExists(data) {
  ...
  try {
    ...
    const getBook = await db
      .collection("books")
      .findOne({ book_uuid: data.book_uuid });
    const getReadingList = await db
      .collection("reading_lists")
      .findOne({ _id: ObjectId(data.reading_list_id) });
...
async function addBookList(data) {
  ...
  try {
    ...
    const updateReadingList = await db
      .collection("reading_lists")
      .updateOne(
...
async function deleteReadingListByID(reading_list_id) {
  ...
  try {
    ...
    const res = await db
      .collection("reading_lists")
      .deleteOne({ _id: ObjectId(reading_list_id) });
...
async function getAllBooksFromReadingList(readingListID) {
  ...
  try {
    ...
    const getBook = await db
      .collection("books")
      .findOne({ book_uuid: readingListID.book_uuid });
